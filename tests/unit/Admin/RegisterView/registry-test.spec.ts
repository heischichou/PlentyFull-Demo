...
describe("Registry View", () => {
    ...
    it("setVisibleButtons() computed property returns the correct number of visible buttons", async () => {
        const wrapper = factory([]);
        const mockSetVisibleButtons = jest.spyOn( wrapper.vm, "setVisibleButtons", "get");
    ...
    it("runs switchTab() method successfully", async () => {
        const wrapper = factory([]);
        const refs = [
            { name: "registration", tab: "Registration" },
            { name: "members", tab: "Members" },
    ...
    it("renders the list of requests correctly", async () => {
        ...
        expect(wrapper.findAll(".request").length).toEqual(wrapper.vm.sortedRequests.length);
    ...
    it("convertToDuration() method successfully returns the correct output", async () => {
        ...
        testCases.forEach((testCase) => {
            callbacks++;
            expect(wrapper.vm.convertToDuration(testCase.createdAt, currentDate)).toEqual(testCase.output);
